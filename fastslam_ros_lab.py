...
class FastSlam_ROS(Node):
    ...
    def __init__(self):
        ...
        self.true_landmarks_pub = self.create_publisher(
            PointCloud2,
            '/fastslam/true_landmarks',
            QoSProfile(depth=1, durability=DurabilityPolicy.TRANSIENT_LOCAL),
        )

        initial_header = Header()
        initial_header.frame_id = "map"
        initial_header.stamp = self.get_clock().now().to_msg()
        self.publish_true_landmarks(initial_header)
